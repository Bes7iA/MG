...
import { preguntar } from './ui-modal.js';
...
export async function resolverCombate(atacante, defensor, permiteRetiro) {
    ...
    await preguntar('💥 ¡Impacto!', `Tu máquina hizo ${dmg1} de daño. HP restante del objetivo: ${Math.max(defensor.hp, 0)}.`, [{ label: 'Continuar', value: true, destacado: true }], false);
    ...
    while (true) {
        if (turno === 'defensor') {
            ...
            await preguntar('💥 ¡Contraataque!', `El enemigo hizo ${dmg} de daño a tu máquina. HP restante: ${Math.max(atacante.hp, 0)}.`, [{ label: 'Continuar', value: true, destacado: true }], false);
            ...
            await preguntar('💥 ¡Impacto!', `Tu máquina hizo ${dmg} de daño. HP restante del objetivo: ${Math.max(defensor.hp, 0)}.`, [{ label: 'Continuar', value: true, destacado: true }], false);
            if (defensor.hp <= 0) { destruirMaquina(defensor); return 'atacanteGana'; }
            turno = 'defensor';
        }
    }
